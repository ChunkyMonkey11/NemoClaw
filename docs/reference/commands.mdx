...
- If OpenShell confirms the sandbox is absent, `destroy` selects one retained record from its registry generation, gateway, create-attempt evidence, and available identity fingerprint.
  If post-create readiness fails, NemoClaw records the returned sandbox identity in the pending reservation.
  `destroy` can use that record when its identity matches the retained recovery record.
  For an older unowned pending reservation with no lifecycle identity, `destroy` can remove only local recovery state after OpenShell confirms absence.
...
If cleanup still fails, the command refuses to rebuild and prints recovery guidance for the reported condition.
Fix that condition, then rerun the command.
This failure prevents the next rebuild from restoring stale channel authentication files.
