---
title:
  page: "Config-Set SSRF Hardening (April 2026)"
  nav: "Config-Set SSRF Hardening"
description:
  main: "Explains the hostname-based SSRF validation gap in `nemoclaw config set`, the security impact, and the DNS-aware hardening changes that close it."
  agent: "Describes the SSRF gap in `nemoclaw config set` where hostnames were not DNS-resolved, then documents the new strict validation path, tests, and rollout talking points for maintainers."
keywords: ["ssrf", "dns rebinding", "config set", "network hardening", "security regression"]
topics: ["generative_ai", "ai_agents"]
tags: ["security", "network_policy", "ssrf", "hardening", "incident_response"]
content:
  type: concept
  difficulty: advanced
