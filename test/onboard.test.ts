...
`;
  ...
  it(
    "non-interactive reuses a ready sandbox when selection config is unreadable",
    { timeout: 60_000 },
    async () => {
      const repoRoot = path.join(import.meta.dirname, "..");
      const tmpDir = fs.mkdtempSync(
        path.join(os.tmpdir(), "nemoclaw-onboard-noninteractive-unreadable-selection-"),
      );
      const fakeBin = path.join(tmpDir, "bin");
      const scriptPath = path.join(tmpDir, "noninteractive-unreadable-selection.js");
      const onboardPath = JSON.stringify(path.join(repoRoot, "dist", "lib", "onboard.js"));
      const runnerPath = JSON.stringify(path.join(repoRoot, "dist", "lib", "runner.js"));
      const registryPath = JSON.stringify(path.join(repoRoot, "dist", "lib", "registry.js"));
...
runner.run = (command, opts = {}) => {
  ...
  if (normalized.includes("sandbox download")) return { status: 1 };
  return { status: 0 };
};
runner.runCapture = (command) => {
  if (_n(command).includes("sandbox get my-assistant")) return "my-assistant";
...
`;
      ...
      assert.ok(
        payload.commands.every((entry) => !entry.command.includes("sandbox create")),
        "should NOT recreate sandbox when selection config is unreadable",
      );
      assert.ok(
        payload.commands.every((entry) => !entry.command.includes("sandbox delete")),
        "should NOT delete sandbox when selection config is unreadable",
      );
      assert.ok(
        result.stdout.includes("Existing provider/model selection is unreadable; reusing sandbox."),
        "should report unreadable selection and safe reuse in non-interactive mode",
      );
    },
  );
